...
void _flw_main() {
    ...
    /*
     * AeroCloakingCore embedded-visual cloak routing.
     *
     * Signed signal:
     *   +0..+1 = DITHER strength
     *   -0..-1 = ALPHA strength
     *
     * ALPHA visuals are completely suppressed during Flywheel's normal pass
     * and rendered only during AeroCloakingCore's second, late translucent
     * pass.  Everything else is discarded during that late pass.
     */
    #ifdef FLW_EMBEDDED
    float aerocloakingcore_signal = clamp(
        flw_vertexAeroCloakStrength,
        -1.0,
        1.0
    );

    bool aerocloakingcore_alpha = aerocloakingcore_signal < -0.0001;

    if (_flw_aeroLateAlphaPass != 0) {
        if (!aerocloakingcore_alpha) {
            discard;
        }

        flw_fragColor.a *= clamp(
            1.0 + aerocloakingcore_signal,
            0.0,
            1.0
        );

        if (flw_fragColor.a <= 0.0001) {
            discard;
        }
    } else {
        if (aerocloakingcore_alpha) {
        ...
        if (aerocloakingcore_signal > 0.0) {
            float aerocloakingcore_threshold = fract(
                52.9829189 *
                fract(
                    dot(
            ...
            if (aerocloakingcore_threshold < aerocloakingcore_signal) {
                discard;
            }
        }
    }
